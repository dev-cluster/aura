        ...
        <v-layout wrap align-start justify-start row fill-height class="mt-0 mb-0">
            <v-flex xs6 sm3 md2 lg2 v-for="(item,i) in TeamDetails" :key="i" class="text-xs-center" style="text-align:center">
                ...
                <div 
                    ...
                    <v-btn class="mt-0 mx-0" icon v-if="(item.email).length>0" :href="item.email" target="_blank">
                        <v-icon small style="color:#fff">email</v-icon>
                    </v-btn>
...
<script>
import TeamDetails from '@/assets/data/pyGoaTeam.json'
export default {
    data() {
        return {
